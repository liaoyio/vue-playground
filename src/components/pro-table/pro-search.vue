...
<template>
  <el-form ref="formRef" :model="form" label-width="auto" label-suffix=":" class="pro-form">
    <el-row :gutter="20">
      <el-col v-for="item in columns" :key="item.prop" :span="6">
        <el-form-item :label="item.label" :prop="item.prop">
          <component :is="elComponentTypeMap[
            ...
            <template v-if="item.searchType === 'select'">
              <el-option v-for="option in item.searchProps.options" :key="option.value" :value="option.value"
                :label="option.label" />
            </template>
